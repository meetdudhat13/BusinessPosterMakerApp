...
class SearchGeneralEventByTitleUseCase @Inject constructor(
    private val repository: GeneralEventRepository,
) {
    suspend operator fun invoke(poster: String) : Resource<List<GeneralEvent>> = repository.searchGeneralEventByTitle(poster)
}