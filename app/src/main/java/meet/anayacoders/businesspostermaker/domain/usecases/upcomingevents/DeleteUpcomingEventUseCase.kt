...
class DeleteUpcomingEventUseCase @Inject constructor(
    private val repository: UpcomingEventRepository,
) {
    suspend operator fun invoke(poster: UpcomingEvent) : Resource<UpcomingEventsResponseDto> = repository.deleteEvent(poster)
}