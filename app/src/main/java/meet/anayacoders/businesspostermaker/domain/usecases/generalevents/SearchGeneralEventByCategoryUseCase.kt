...
class SearchGeneralEventByCategoryUseCase @Inject constructor(
    private val repository: GeneralEventRepository,
) {
    suspend operator fun invoke(poster: String) : Resource<List<GeneralEvent>> = repository.searchGeneralEventByCategory(poster)
}