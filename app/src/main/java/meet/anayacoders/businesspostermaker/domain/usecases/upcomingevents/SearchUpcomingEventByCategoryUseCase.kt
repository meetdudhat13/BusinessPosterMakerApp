...
class SearchUpcomingEventByCategoryUseCase @Inject constructor(
    private val repository: UpcomingEventRepository,
) {
    suspend operator fun invoke(poster: String) : Resource<List<UpcomingEvent>> = repository.searchUpcomingEventByCategory(poster)
}