...
class UpdateGeneralEventUseCase @Inject constructor(
    private val repository: GeneralEventRepository,
) {
    suspend operator fun invoke(poster: GeneralEvent) : Resource<GeneralEventsResponseDto> = repository.updateEvent(poster)
}