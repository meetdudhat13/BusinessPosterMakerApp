...
class SearchPosterByCategoryUseCase @Inject constructor(
    private val repository: PosterRepository,
) {
    suspend operator fun invoke(poster: String) : Resource<List<Poster>> = repository.searchPosterByCategory(poster)
}